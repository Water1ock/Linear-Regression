...
for qwerty in range(20):
    ...
    theta_history = []
    theta, cost_history = stochastic_grad_descent(X_train, Y_train, theta, 0.001, 20)
    ...
    best_cost_001.append(cost_funct(theta, X_test, Y_test))
    ...
    theta = theta.transpose()
    theta_history = []
    theta, cost_history = stochastic_grad_descent(X_train, Y_train, theta, 0.01, 20)
    ...
    best_cost_01.append(cost_funct(theta, X_test, Y_test))

    cost_history = []
    theta = np.asmatrix(initial)
    theta = theta.transpose()
    theta_history = []
    theta, cost_history = stochastic_grad_descent(X_train, Y_train, theta, 0.1, 20)
    ...
    best_cost_1.append(cost_funct(theta, X_test, Y_test))
...
i2 = -1
i3 = -1
for i in range(len(best_cost_001)):
    if best_cost_001[i] == min(best_cost_001):
        index = i
        i1 = index
        break
...
index = -1
for i in range(len(best_cost_01)):
    if best_cost_01[i] == min(best_cost_01):
        index = i
        i2=index
        break
...
index = -1
for i in range(len(best_cost_1)):
    if best_cost_1[i] == min(best_cost_1):
        index = i
        i3 = index
        break
...
print("Minimum Value of Cost Function obtained with alpha as 0.001 on testing data = " + str(min(best_cost_001)))
...
print("")
print("")
print("Minimum Value of Cost Function obtained with alpha as 0.01 on testing data = " + str(min(best_cost_01)))
...
print("")
print("")
print("Minimum Value of Cost Function obtained with alpha as 0.1 on testing data = " + str(min(best_cost_1)))
